    ...
    const handleBanToggle = async (userId: string, currentlyBanned: boolean) => {
        ...
        try {
            ...
            if (response.ok) {
                setUsers((prev) =>
                    prev.map((u) =>
                        u.id === userId ? { ...u, isBanned: data.isBanned } : u
                    )
                );
        ...
        <section className="border border-border rounded-xl p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
                ...
                <Select
                    value={filter}
                    onValueChange={(v) => {
                        setFilter(v as "all" | "active" | "banned");
                        setPagination((prev) => ({ ...prev, page: 1 }));
                    }}
                >
                                ...
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {user.isAdmin && (
                                                <span className="flex items-center gap-1 px-2 py-0.5 text-xs bg-amber-500 text-white rounded-full">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {user.provider} · Joined{" "}
                                            {new Date(user.createdAt).toLocaleDateString()}
                                        </p>
                                    </div>
                                </div>
                                {user.isAdmin ? (
                                    <span className="text-xs text-muted-foreground italic self-start sm:self-auto">
                                        Protected
                                    </span>
                                ) : (
                                    <button
