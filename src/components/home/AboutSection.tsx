...
const GoalItem: React.FC<GoalItemProps> = ({ text, done = false }) => (
    <div className="flex items-center gap-2 border border-border rounded-lg bg-muted p-2">
        ...
        {/* Text */}
        <p
            className={`text-sm ${done ? "line-through text-secondary-foreground" : "text-foreground/90"}`}
        >
            {text}
        </p>
    </div>
...
const CardHeadline: React.FC<CardHeadlineProps> = ({ title, subtitle }) => (
    <div className="mt-2">
