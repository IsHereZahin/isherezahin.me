    ...
    return (
        <div className="py-2">
            <div className="mb-1.5 flex items-center justify-between gap-3 text-[13px]">
                <span className="truncate font-medium text-[#26262B]">{label}</span>
                <span className="shrink-0 tabular-nums text-[#9a978f]">{fmtFull(value)}</span>
            </div>
...
function Empty({ label }: { label: string }) {
    return <p className="py-6 text-center text-[12px] text-[#9a978f]">{label}</p>;
}
        ...
        <div className={`${CARD} flex flex-col`}>
            <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#F4C63D]/20">
                    <MonitorSmartphone className="h-5 w-5 text-[#26262B]" />
                </span>
                <div>
                    <h3 className="text-[16px] font-semibold text-[#26262B]">Audience</h3>
                    <p className="mt-0.5 text-[13px] text-[#9a978f]">Devices &amp; locations</p>
                </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-x-6 sm:grid-cols-2">
                ...
                <div className="min-w-0">
                    <p className="mb-1 text-[12px] font-semibold uppercase tracking-wide text-[#9a978f]">Devices</p>
                    {devices.length === 0 ? (
                ...
                {/* Countries */}
                <div className="mt-4 min-w-0 border-t border-[#f1ede5] pt-4 sm:mt-0 sm:border-l sm:border-t-0 sm:pl-6 sm:pt-0">
                    <p className="mb-1 flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-wide text-[#9a978f]">
