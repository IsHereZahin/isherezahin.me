    ...
    if (isLoading || !status) {
        return (
            <div className="flex items-center justify-center py-24">
                <div className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--s-invert)] border-t-transparent" />
            </div>
        );
    }
...
function GateShell({
    ...
    return (
        <div className="flex min-h-[60vh] items-center justify-center px-4 py-10">
            <div className="w-full max-w-md rounded-[28px] border border-[var(--s-border)] bg-[var(--s-card)] p-8 text-center shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--s-invert)] text-white">
                    {icon}
                </div>
                <h3 className="text-[20px] font-semibold text-[var(--s-text)]">{title}</h3>
                <p className="mx-auto mt-2 max-w-sm text-[13px] leading-relaxed text-[var(--s-muted)]">{subtitle}</p>
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-3 text-left">
                <div className="relative">
                    <KeyRound className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--s-faint)]" />
                    <input
                        type="password"
                        ...
                        autoFocus
                        className="h-11 w-full rounded-full border border-[var(--s-border)] bg-[var(--s-soft)] pl-10 pr-4 text-[13px] text-[var(--s-text)] outline-none transition-colors placeholder:text-[var(--s-faint)] focus:border-[var(--s-invert)] focus:bg-[var(--s-card)]"
                    />
                </div>
                <button
                    type="submit"
                    disabled={submitting}
                    className="flex h-11 w-full items-center justify-center rounded-full bg-[var(--s-invert)] text-[13px] font-medium text-white transition-transform hover:scale-[1.01] disabled:opacity-60"
                >
