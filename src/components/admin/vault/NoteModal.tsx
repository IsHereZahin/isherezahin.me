            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="mt-1 flex gap-1.5">
                            {TYPE_OPTIONS.map((opt) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-xs rounded-md transition-colors ${type === opt.value ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
                                >
                                    {opt.label}
                                </button>
                            ))}
                    ...
                    <div className="space-y-2">
                        ...
                        {checklist.map((item, i) => (
                            <div key={i} className="flex items-center gap-2">
                                ...
                                />
                                <button type="button" onClick={() => setChecklist((prev) => prev.filter((_, idx) => idx !== i))} className="text-muted-foreground hover:text-destructive">
                                    <Trash2 className="h-4 w-4" />
                                </button>
                            </div>
                        ))}
                        <button type="button" onClick={() => setChecklist((prev) => [...prev, { text: "", done: false }])} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                ...
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <p className="text-sm font-medium">Encrypt this note</p>
                            <p className="text-xs text-muted-foreground">Stored encrypted at rest with your vault key.</p>
                        </div>
                    </div>
